...
export default function SentryModule (moduleOptions) {
  ...
  const defaults = {
    ...
    /** Enable Sentry APM on server-side */
    apm: false,
    // @ts-ignore
  ...
  if (serverSentryEnabled(options)) {
    ...
    // APM Tracing - start
    // See: https://docs.sentry.io/performance-monitoring/getting-started/?platform=node
    if (options.apm) {
      // @ts-ignore
      this.nuxt.hook('render:setupMiddleware', app => app.use(SentryHandlers.tracingHandler()))
    }
    // APM Tracing - end
  }
  ...
  if (serverSentryEnabled(options)) {
    this.nuxt.hook('ready', () => initializeServerSentry(this, options, logger))
  }
